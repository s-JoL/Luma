...
Agent 默认工具：`generate_image`、`edit_image`、`generate_video`，由 profile
选中的模型决定。没有 profile 时回落到已配置密钥的托管模型，而不是「排序第一
且无需密钥」的本地 Comfy——后者在 8188 没人听的时候会让对话里的生图静默失败。
创作台 `GET /studio/tools` 默认选预设绑过的模型；没有绑定才按
...
预设绑定优先于「哪个后端还活着」：绑了本机 ComfyUI 而它没启动，对话里的生图
